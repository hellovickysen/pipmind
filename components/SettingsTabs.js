    ...
    <div className="space-y-6">
      ...
      <div className={card}>
        ...
        <button
          ...
          disabled={pwSaving}
          className="mt-4 rounded-xl px-5 py-2.5 text-sm font-semibold text-[#08080f] disabled:opacity-60"
...
function JournalSettingsTab({ prefs, onSaved }) {
  const existing = (prefs && prefs.custom_emotions && prefs.custom_emotions.length > 0)
    ? prefs.custom_emotions
    : DEFAULT_EMOTIONS;
  const [emotions, setEmotions] = useState(existing);
  ...
  function addEmotion() {
    ...
    if (!trimmed) return;
    if (emotions.map((e) => e.toLowerCase()).includes(trimmed.toLowerCase())) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={card}>
        ...
        <p className="mb-4 text-xs text-white/40">Customize the feelings you track in your journal. These appear on every trade entry.</p>
          ...
          {emotions.map((em, i) => (
            <span key={i} className="group flex items-center gap-1.5 rounded-full border border-violet-400/30 bg-violet-500/10 px-3 py-1.5 text-xs text-violet-200">
              ...
              <button
                ...
                className="hidden text-red-400 hover:text-red-300 group-hover:inline"
              >
                ✕
              </button>
            </span>
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="e.g. Anxious, Excited, Frustrated…"
            onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addEmotion())}
          />
          <button
            type="button"
            onClick={addEmotion}
            className="rounded-lg border border-white/15 bg-white/5 px-4 py-2.5 text-sm text-white/70 hover:text-white"
          >
            Add
          </button>
        </div>

        <button type="button" onClick={resetDefaults} className="mt-3 text-xs text-white/30 underline hover:text-white/50">
          Reset to defaults
        </button>
      </div>
      ...
      <div className={card}>
        ...
        <p className="mb-4 text-xs text-white/40">Pre-fill confidence rating when logging a new trade. Set to 0 for no default.</p>
          ...
          {[0, 1, 2, 3, 4, 5].map((i) =>
            i === 0 ? (
              <button
                ...
                className={'rounded-lg border px-2 py-1 text-xs ' + (defaultConfidence === 0 ? 'border-cyan-400/50 bg-cyan-500/10 text-cyan-300' : 'border-white/10 text-white/30')}
              >
                None
              </button>
            ) : (
              <button key={i} type="button" onClick={() => setDefaultConfidence(i)} className={i <= defaultConfidence ? 'text-amber-400' : 'text-white/20'}>
                &#9733;
              </button>
            )
          )}
  ...
  return (
    <div>
      ...
      {tab === 'journal' && (
        <JournalSettingsTab
          prefs={prefs}
          onSaved={(updated) => setPrefs((p) => ({ ...p, ...updated }))}
        />
      )}
    </div>
  );
