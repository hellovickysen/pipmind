...
export default function ConfirmDialog({ open, onClose, onConfirm, title, message, confirmLabel, loading }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="mx-4 w-full max-w-sm rounded-2xl border border-white/10 bg-[#0e0e18] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        {/* Warning icon */}
        ...
        <h2 className="text-center font-display text-lg font-bold">{title || 'Are you sure?'}</h2>
        <p className="mt-2 text-center text-sm text-white/55">{message || "This action can't be undone."}</p>
