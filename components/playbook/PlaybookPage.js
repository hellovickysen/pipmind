...
import { PlaybookEmptyIcon } from '@/components/ui/EmptyStates';
...
export default function PlaybookPage({ setups }) {
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-white/55">
            {activeSetups.length} active setup{activeSetups.length !== 1 ? 's' : ''} in your playbook
          </p>
        </div>
        <button
          onClick={() => setEditing('new')}
