...
const FEATURES = [
  ...
  { icon: '💰', title: 'Prop firm expense tracker', desc: 'Track challenge fees, payouts, and ROI across all your prop firms. Know exactly where your money goes.', coming: true },
  ...
  { icon: '🔥', title: 'Daily streak & achievements', desc: 'Gamified journaling streaks, weekly achievements, and progress badges to keep you consistent.', coming: true },
    ...
    <main className="min-h-screen">
      ...
      <nav className="flex items-center justify-between px-6 py-5 sm:px-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login?mode=signup" className="rounded-xl px-4 py-2 text-sm font-semibold text-[#08080f]" style={gradientBtn}>
            Join the beta
          </Link>
        </div>
      </nav>
      ...
      <section className="relative overflow-hidden px-6 pb-20 pt-16 text-center sm:px-10 sm:pt-24">
        ...
        <div className="relative z-10 mx-auto max-w-3xl">
          <div className="mb-6 inline-block rounded-full border border-violet-400/30 bg-violet-500/10 px-4 py-1.5 text-xs font-semibold text-violet-200">
          ...
          <h1 className="font-display text-4xl font-extrabold leading-tight sm:text-5xl lg:text-6xl">
            ...
            <span style={gradientText}>why you lose challenges.</span>
          </h1>

          <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-white/60 sm:text-lg">
            PropJournal is an AI-powered trading journal built for prop firm traders. Log trades, track your psychology, manage challenge expenses, and get coaching that finds the one pattern costing you funded accounts.
          </p>

          <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <Link href="/login?mode=signup" className="rounded-xl px-8 py-3.5 text-base font-semibold text-[#08080f] transition-transform hover:-translate-y-0.5" style={gradientBtn}>
              Start journaling free →
            </Link>
            <Link href="/login" className="rounded-xl border border-white/15 bg-white/5 px-8 py-3.5 text-base font-semibold text-white/70 hover:text-white">
              Log in
            </Link>
          </div>
      ...
      <section className="px-6 py-16 sm:px-10">
        <div className="mx-auto max-w-4xl">
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              { num: '85%', label: 'of prop firm challenges fail', sub: 'Most failures are discipline failures, not strategy failures.' },
              { num: '$200+', label: 'per failed challenge', sub: 'The cost of repeating the same mistakes without knowing what they are.' },
              { num: '1', label: 'pattern to fix', sub: 'Most traders have one core leak. PropJournal\'s AI finds it.' },
            ].map((s, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-center">
                <div className="font-display text-3xl font-extrabold" style={gradientText}>{s.num}</div>
      ...
      <section className="px-6 py-16 sm:px-10">
        <div className="mx-auto max-w-5xl">
          <div className="text-center">
            ...
            <p className="mx-auto mt-3 max-w-lg text-sm text-white/50">All features are free during beta. Coming soon features are actively being built.</p>
          </div>
            ...
            {FEATURES.map((f, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  {f.coming ? (
                    <span className="rounded-full border border-amber-400/30 bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold text-amber-300">Coming soon</span>
                  ) : (
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-left">
      ...
      <section className="px-6 py-20 sm:px-10">
        <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 p-10 text-center" style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(34,211,238,0.05))' }}>
          <h2 className="font-display text-2xl font-bold sm:text-3xl">
            Stop guessing. <span style={gradientText}>Start knowing.</span>
          </h2>
          <p className="mx-auto mt-4 max-w-md text-sm text-white/55">
            Join the PropJournal beta — every feature is free while we build. Be one of the first traders to get AI coaching that actually helps you pass challenges.
          </p>
          <Link href="/login?mode=signup" className="mt-8 inline-block rounded-xl px-8 py-3.5 text-base font-semibold text-[#08080f] transition-transform hover:-translate-y-0.5" style={gradientBtn}>
